...
public class AIResponseEngine {
    ...
    public static LearningPath createPersonalizedPath(Student student, List<Course> availableCourses) {
        LearningPath path = new LearningPath(
                "LP" + UUID.randomUUID().toString().substring(0, 8),
                "Personalized Path for " + student.getUsername(),
                "Custom learning path based on student progress and available courses");

        // Add recommended courses to the path
        List<Course> recommendations = recommendCourses(student, availableCourses);
